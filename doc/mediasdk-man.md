...
With joined sessions, the application can set the priority of session operations through the [MFXSetPriority](#MFXSetPriority) function. A lower priority session receives less CPU cycles and make the hardware accelerater task priority lower during hardware accelerater scheduling.
